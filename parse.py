...
def parse_xml(xml_file, only_one=True):
    ...
    for reading_session in reading_sessions:
        ...
        for nodule in nodules:
            ...
            line = [
                [imsop_uid, ser_ist_id, nodule_id, x_center, y_center, diameter, malignacy, x_min, y_min, x_max, y_max]]
            extended_line = [patient_id, imsop_uid, nodule_id, x_diameter, y_diameter, malignacy, x_min, y_min, x_max,
                             y_max, sphericiy, margin, spiculation, texture, calcification, internal_structure,
                             lobulation, subtlety]
            pos_lines.append(line)
            extended_lines.append(extended_line)

    # 去除不同医生对同一张ct图像的标注，只保留一个医生的标注
    if only_one:
...
def save_csv(pos_dict):
    for uid, nodule_info in pos_dict.items():
        ...
        df_annos.to_csv(settings.EXTRACTED_IMAGE_DIR + uid + ".csv", index=False)
